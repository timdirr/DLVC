...
class YourCNN2(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.layer_2     = torch.nn.Conv2d(self.cs,self.cs*2,(5,5), padding = "same") # input size (B, 16, 32, 32)
        self.bn_2        = torch.nn.BatchNorm2d(self.cs*2)
        
        self.maxp        = torch.nn.MaxPool2d((2,2),stride = (2,2))
        
        self.layer_3     = torch.nn.Conv2d(self.cs*2,self.cs*4,(3,3), padding = "same") # input size (B, 16, 16, 16)
        self.bn_3        = torch.nn.BatchNorm2d(self.cs*4)
        
        self.layer_4     = torch.nn.Conv2d(self.cs*4,self.cs*4,(3,3), padding = "same") # input size (B, 32, 16, 16)
        self.bn_4        = torch.nn.BatchNorm2d(self.cs*4)
        
        self.maxp2       = torch.nn.MaxPool2d((2,2),stride = (2,2))
        
        self.layer_5     = torch.nn.Conv2d(self.cs*4,self.cs*8,(3,3), padding = "same") # input size (B, 32, 8, 8)
        self.bn_5        = torch.nn.BatchNorm2d(self.cs*8)
        self.drop_5      = torch.nn.Dropout(0.25)
        
        self.layer_6     = torch.nn.Linear(self.cs*8*8*8, 128) # input 8192
        self.bn_6        = torch.nn.BatchNorm1d(128)
        self.drop_6      = torch.nn.Dropout(0.25)
