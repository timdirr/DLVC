...
def train(args):
    ...
    if args.dataset == "oxford":
        ...
        # print(f"number training images: {len(train_data)}")
        # print(f"number validation images: {len(val_data)}")
...
if __name__ == "__main__":
    ...
    if CONFIG["load_path"] == "default" and args.dataset == "oxford":
        ...
        config_str_tmp = config_str.replace("oxford", "city")
        config_str_tmp = config_str_tmp.replace(str(CONFIG["lr"]), "6e-05")
        
        CONFIG["load_path"] = os.path.join(
            "training", "SegFormer", config_str_tmp, "SegFormer_model.pth")
